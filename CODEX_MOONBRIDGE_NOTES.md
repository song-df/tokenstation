...
> 记录 Codex app 接入 `ai.aiotedu.cc`(new-api 网关)时遇到的协议问题、根因、验证过程与解决方案。

---

## 1. 现象

在 Codex app 里把 provider 直接指向 `ai.aiotedu.cc`,发消息报错:

```
unexpected status 404 Not Found: Invalid URL (GET /v1/responses),
url: http://ai.aiotedu.cc/v1/responses
```
...
| `POST ai.aiotedu.cc/v1/responses` | **404**（new-api 没有）|
| `POST ai.aiotedu.cc/v1/chat/completions` | 200 ✅（工具调用 `finish_reason=tool_calls` 正常）|
...
Codex app (Responses 协议)
  ...
  → provider=aiotedu → https://ai.aiotedu.cc (new-api)
  → deepseek-v4-pro
```
...
providers:
  aiotedu:
    base_url: "https://ai.aiotedu.cc"        # 生产 new-api,中继在 /v1
...
（CC Switch 里配 "Claude Code" 这个工具填上述即可,或手动设 `ANTHROPIC_BASE_URL=https://ai.aiotedu.cc`、`ANTHROPIC_AUTH_TOKEN=sk-...`、`ANTHROPIC_MODEL=claude-opus-4-8`。）
...
| **Claude Code** | Anthropic Messages | ✅ 可以 | 直接配 `ai.aiotedu.cc` |
| curl / OpenAI SDK | Chat Completions | ✅ 可以 | `ai.aiotedu.cc/v1/chat/completions` |
