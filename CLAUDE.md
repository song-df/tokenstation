...
- **Anthropic-native**: `POST /v1/messages`, `GET /v1/models/anthropic`,
  `POST /v1/messages/count_tokens` — used by Claude Code.
...
`_MODEL_MAP` in `relay.py` maps client-facing model IDs (e.g. `claude-opus-4-8`) to upstream
IDs (e.g. `anthropic/claude-opus-4.8`). Update this when adding/renaming models.
...
`x-api-key` header (Claude Code convention), then tries, in order:
